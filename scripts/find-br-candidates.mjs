...
import { hasJavascriptAnchors } from './lib/convert-supported-html.mjs';
...
const hasBreaks = (value = '') => /<br\s*\/?>/i.test(value);

const main = () => {
	const [, , providedLimit] = process.argv;
	const limit = Number.isFinite(Number(providedLimit)) ? Number(providedLimit) : 50;
	const fileNames = fs.readdirSync(POSTS_DIR).filter((fileName) => fileName.endsWith('.md')).sort();
	...
	const candidates = fileNames
		...
		.map((post) => ({
			...
			brCount: ((`${post.content}\n${post.excerpt}`).match(/<br\s*\/?>/gi) || []).length,
		}));

	console.log(
		JSON.stringify(
			{ count: candidates.length, candidates: candidates.slice(0, limit) },
			null,
			2,
		),
	);
