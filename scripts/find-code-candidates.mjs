...
const hasCodeMarkup = (value = '') => /<pre\b|<\/pre>|<code\b|<\/code>/i.test(value);
...
const main = () => {
	...
	const candidates = fileNames
		...
		.filter((post) => hasOnlyPlainCodeTags(post.content))
		.filter((post) => hasOnlyPlainCodeTags(post.excerpt))
		...
		.map((post) => ({
			...
			codeCount: ((`${post.content}\n${post.excerpt}`).match(/<(?:pre|code)\b/gi) || []).length,
		}));
