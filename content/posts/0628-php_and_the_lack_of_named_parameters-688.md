...
excerpt: >-
  ...
  API input parameters to methods in my code. For instance, I have an API for
  John VanOrange that allows anyone to access most of the site's functionality.
keywords: >-
...
For instance, I have an [API](https://github.com/cbulock/JohnVanOrange/wiki/API) for [John VanOrange](https://github.com/cbulock/JohnVanOrange) that allows anyone to access most of the site's functionality. The public API methods all map to classes and functions inside those classes. Each of these methods allow for various parameters to be passed into them.
...
$.ajax({
	type: 'post',
	url: 'http://johnvanorange.com/api/tag/add',
	data: {
		name: 'New tag',
		image: 'xJh5De'
	}
...
<?
$class_obj = new $class;
return $class_obj->{$method}($params);
```
...
I figured there had to be a better way. There is [call_user_func](http://php.net/manual/en/function.call-user-func.php) and [call_user_func_array](http://www.php.net/manual/en/function.call-user-func-array.php) functions that allows calling functions passing the parameters if they are in an array. But, again, that requires an indexed array that already knows what order the parameters need to be in. What I really need is a `call_user_func_assoc_array` that maps an associative array to the parameters within a function. But, alas, that doesn't exist.

There is hope however. PHP 5 introduced the [Reflection API](http://www.php.net/manual/en/book.reflection.php). Honestly, I'm not that familiar with it, but after reading up on it, I learned it could solve my problem.
...
foreach($reflectParams as $param) {
  if (isset($params[$param->getName()])) {
   $result_params = []; = $params[$param->getName()];
  } else {
   if ($param->isOptional()) {
    $result_params = []; = $param->getDefaultValue();
   } else {
    $result_params = []; = NULL;
   }
  }
...
return $reflectMethod->invokeArgs($class_obj, $result_params);
```
